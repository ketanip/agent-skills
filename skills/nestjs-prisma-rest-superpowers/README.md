...
> Part of the [ketanip/agent-skills](https://github.com/ketanip/agent-skills) collection — drop the `--skill` flag to install every skill in it. Works with Claude Code, Cursor, Copilot, Windsurf, Cline — any agent that supports [skills.sh](https://skills.sh).
...
Taking the whole collection costs nothing — each skill only activates on work it's relevant to, so nothing else touches your context while you're writing NestJS.

Prefer to vendor by hand? Copy `skills/nestjs-prisma-rest-superpowers/` into your project's `.claude/skills/` (or `~/.claude/skills/` for every project).
...
| **Agent compatibility** | Claude Code, Cursor, Copilot, Windsurf, Cline, and any agent that supports skills.sh |
...
Open an issue or PR at [github.com/ketanip/agent-skills](https://github.com/ketanip/agent-skills) — contributions welcome.
